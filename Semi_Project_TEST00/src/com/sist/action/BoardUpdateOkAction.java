...
public class BoardUpdateOkAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		int article_no = Integer.parseInt(multi.getParameter("article_no"));
		...
		System.out.println(article_no + " : " + article_writer + " : " + article_gen1 + " : " + article_title + " : "
				+ article_cont + " : " + article_file1);
		...
		int result = aDao.updateArticle(aVo);

		
		if (result == 1) {
			url = "article_cont.jsp";
		}
		 
		ActionForward forward = new ActionForward();
		ArticleVO vo = aDao.articleCont(article_no);
		...
		/*
		 * forward.setRedirect(true); forward.setPath(url);
		 */
